...
// Derived from the  GNU ISO C++ Library
...
// forward definition
template <class _Val, class _Key, class _HasfhFcn, class _ExtractKey, class _EqualKey, class _Alloc >
class hashtable;
...
private:
  ...
  _Node* _M_get_node()           { return _M_node_allocator.allocate(1,NULL); }
...
public:
  ...
  size_type elems_in_bucket(size_type __bucket) const
  {
    ...
    for (_Node* __cur = _M_buckets[__bucket]; __cur; __cur = __cur->_M_next) __result += 1;
    return __result;
  }
...
void hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::erase(iterator __first, iterator __last)
{
  ...
  if (__first._M_cur == __last._M_cur) return;
  else if (__f_bucket == __l_bucket) _M_erase_bucket(__f_bucket, __first._M_cur, __last._M_cur);
  else {
    _M_erase_bucket(__f_bucket, __first._M_cur, 0);
    for (size_type __n = __f_bucket + 1; __n < __l_bucket; ++__n) _M_erase_bucket(__n, 0);
    if (__l_bucket != _M_buckets.size()) _M_erase_bucket(__l_bucket, __last._M_cur);
  }
}

template <class _Val, class _Key, class _HF, class _Ex, class _Eq, class _All>
inline void hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::erase(const_iterator __first, const_iterator __last)
{
...
void hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::resize(size_type __num_elements_hint)
{
  ...
  if (__num_elements_hint > __old_n) {
    ...
    if (__n > __old_n) {
      std::vector<_Node*> __tmp(__n,(_Node*)(0),_M_buckets.get_allocator());
      try {
