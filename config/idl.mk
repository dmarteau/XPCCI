...
TYPELIBIN = $(topsrcdir)/cci/idl-parser/typelib.py
XPTBIN    = $(topsrcdir)/cci/idl-parser/xpt.py
...
$(EXPORT_DIR)/%.h : %.idl
	@echo $<
	@mkdir -p $(IDL_PYCACHE)
	@python $(PYIDLBIN) $(IDL_FLAGS) $(IDL_INCLUDES) -xpt $(STAGE_DIR)/$(basename $<).xpt  $< > "$@"
...
$(STAGE_DIR)/%.xpt : %.idl
	@echo $<
	@mkdir -p $(IDL_PYCACHE)
	@python $(TYPELIBIN) $(IDL_FLAGS) $(IDL_INCLUDES) $< -o "$@"
